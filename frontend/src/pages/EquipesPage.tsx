...
export default function EquipesPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      await updateEquipe(editingId, nome.trim(), cidade.trim());
    } else {
      await addEquipe(nome.trim(), cidade.trim());
    }
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Excluir equipe e todos os seus membros?")) { 
      await deleteEquipe(id); 
      refresh(); 
      if (viewingEquipe?.id === id) setViewingEquipe(null); 
    }
  };

  const openMembers = async (eq: Equipe) => {
    setViewingEquipe(eq);
    const data = await getParticipantes(eq.id);
    setMembros(data);
  ...
  const refreshMembers = async () => {
    if (viewingEquipe) {
      const data = await getParticipantes(viewingEquipe.id);
      setMembros(data);
    }
  };
  ...
  const handleMemberSubmit = async (e: React.FormEvent) => {
    ...
    if (editingMemberId) {
      const err = await updateParticipante(editingMemberId, membroNome.trim(), cpfClean, viewingEquipe!.id);
      if (typeof err === "string") { setMemberError(err); return; }
    } else {
      const res = await addParticipante(membroNome.trim(), cpfClean, viewingEquipe!.id);
