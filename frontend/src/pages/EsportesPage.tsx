...
export default function EsportesPage() {
  ...
  // Carrega esportes ao montar
  useEffect(() => {
    const loadEsportes = async () => {
      setLoading(true);
      const data = await getEsportes();
      setEsportes(data);
      setLoading(false);
    };
    loadEsportes();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (editingId) {
      const err = await updateEsporte(editingId, modalidade, nome.trim(), data, horario.trim());
      if (typeof err === "string") { setError(err); return; }
    } else {
      const res = await addEsporte(modalidade, nome.trim(), data, horario.trim());
  ...
  const handleDelete = async (id: number) => {
    if (confirm("Excluir esporte?")) { 
      await deleteEsporte(id); 
      refresh(); 
      if (managingEsporte?.id === id) setManagingEsporte(null); 
    }
  };

  const openManage = async (esp: Esporte) => {
  ...
  const refreshParticipacoes = async (esporteId: number) => {
    const parts = await getParticipacoes(esporteId);
    const allParts = await getParticipantes();
    const allEquipes = await getEquipes();
  ...
  const handleSelectEquipe = async (eqId: number) => {
    setSelectedEquipeId(eqId);
    const parts = await getParticipantes(eqId);
    setEquipeParts(parts);
    setChecked(new Set());
  };
  ...
  const handleSaveParticipantes = async () => {
    ...
    await addParticipacoes(managingEsporte.id, Array.from(checked));
    setChecked(new Set());
    refreshParticipacoes(managingEsporte.id);
  };

  const handleRemoveParticipacao = async (id: number) => {
    await removeParticipacao(id);
