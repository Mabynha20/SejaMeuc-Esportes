...
export default function RelatoriosPage() {
  ...
  const esporteData = useMemo(() => {
    ...
    const parts = getParticipacoes(selectedEsporte.id);
    const allParts = getParticipantes();
    const results = getResultados(selectedEsporte.id);
    ...
    for (const p of parts) {
      ...
      if (!byEquipe.has(p.equipeId)) {
        const res = results.find((r) => r.equipeId === p.equipeId);
    ...
    return Array.from(byEquipe.entries()).map(([eqId, d]) => ({ equipeId: eqId, ...d }));
  ...
  const handlePontosSave = (equipeId: number) => {
    const val = editingPontos.get(equipeId);
    if (val === undefined || !selectedEsporte) return;
    setResultado(equipeId, selectedEsporte.id, parseInt(val) || 0);
